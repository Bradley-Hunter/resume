...
I have a repo with no code in it. No source files, no tests, no CI config, and no package manifests. It holds project ideas, design documents, architecture decisions, and developer profiles. Four projects live there now, ranging from a handful of docs to over a dozen each, and none of them started as more than a sentence in my head.
...
About a year ago I tried to build a metroidvania. I'd been playing Hollow Knight and wanted to make something in that space, so I opened Godot, started prototyping movement and physics, and immediately hit friction. The double jump didn't feel right and the physics weren't behaving the way I wanted. I spent my time fighting the engine instead of designing the game.

But the deeper problem wasn't Godot. It was that I had no idea what I was building. There was no story, no world structure, and no plan for what the player could do beyond move and jump. I was prototyping toward nothing, and the general dissatisfaction with the engine gave me an easy excuse to stop. I abandoned it.
...
Over the past few months I'd used Claude to plan three other projects: Codex (an offline Android ePub reader), a single-player card game, and a RON schema validator. Each one started the same way, where I showed up with a rough idea and left with design documents detailed enough to build from.
...
I mentioned early on that I wanted a story inspired by Percy Jackson, set in Greco-Roman mythology, and I wanted shapeshifting to be a core mechanic. We landed on a child of Hecate as the protagonist, which fit both the mythology angle and the shapeshifting abilities. From there, ideas started pouring out. Some were mine and some were Claude's suggestions that I reacted to. The useful part wasn't that AI had better ideas than me. It was that I had something to respond to instead of staring at a blank page trying to pull a fully formed concept out of thin air.

## The pushback that made it better

Not everything I wanted made it into the design. Early on I said I wanted procedurally generated biomes, and Claude pushed back hard on this. It pointed out that procedural generation fundamentally conflicts with the hand-crafted world design that defines the metroidvania genre. Dead Cells was on my inspiration list, but Dead Cells is a roguelite first and a metroidvania second. Procedural biomes would undermine the exploration, the sense of place, and the interconnected shortcuts that make a metroidvania feel like a metroidvania.

I didn't drop the idea entirely because I liked the unpredictability that procedural generation creates. So I came back with a compromise: hand-crafted biomes connected by procedurally generated labyrinths. The biomes themselves would be designed, but the routes between them would shift.
...
The final design was better than either of our starting positions. I wouldn't have arrived at it alone, and Claude wouldn't have proposed it without me insisting that some element of procedural generation belonged in the game. This is what AI planning actually looks like. Not "AI writes my design docs," but a conversation where both sides contribute, push back, and build on each other.

## Clarifying what I actually mean

The pushback wasn't one-directional, though. The other side of working with AI is the miscommunication. Sometimes I'd describe something I wanted and Claude would interpret it differently than I intended. Other times it would agree too quickly with a surface-level version of my idea without digging into what I actually meant.

I had to re-explain things and go back over decisions. I'd say "that's not what I meant, here's what I'm actually thinking" and walk through it again until the document reflected my intent, not Claude's first interpretation of my intent. This happened more than once across every project. AI is fast at generating structured output, but speed doesn't help if the output captures the wrong thing.

## The quality gate problem

The miscommunication issue connects directly to the biggest recurring problem I ran into. Claude would write a design document and move on, treating it as done, when the document didn't actually capture what we'd discussed. The docs would be thin, skimming over decisions we'd spent real time talking through, with key details dropped and nuance flattened into generic statements.
...
The planning process only works if you treat AI output as a first draft, not a finished product. Every document in my repo went through at least one round of "no, go back, that's not right," and most went through several.
