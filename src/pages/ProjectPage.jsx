...
export default function ProjectPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <h1 className="text-3xl sm:text-4xl font-bold text-white dark:text-white mt-4 mb-1">
        {project.title}
      </h1>
      {project.date && (
        <p className="text-sm text-gray-300 dark:text-gray-500 mb-4">Started {project.date}</p>
      )}
      ...
      {project.github && (
        <div className="mb-8">
          <a
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-700 dark:bg-gray-100 text-white dark:text-gray-900 text-sm font-medium hover:opacity-90 transition-opacity"
          >
          ...
          {project.githubNote && (
            <p className="mt-1.5 text-xs text-gray-300 dark:text-gray-500">{project.githubNote}</p>
          )}
        </div>
      )}
      ...
      {/* Bottom CTA */}
      <div className="flex items-center justify-between mt-12 pt-8 border-t border-gray-400 dark:border-gray-800">
        <Link
          to="/projects"
        ...
        <div className="flex items-center gap-4">
          {relatedPosts.length > 0 && (
            <Link
              to={`/blog?tag=${slug}`}
              className="text-sm text-gray-300 dark:text-gray-400 hover:text-white dark:hover:text-white transition-colors"
            >
              Blog posts ({relatedPosts.length})
            </Link>
          )}
          <Link
            to="/contact"
